            ...
            <div className="mb-3 flex items-start justify-between gap-3">
                ...
                <div className="text-right text-sm">
                    ...
                    <span className="font-semibold text-gray-900 dark:text-white">
                        {formatAmount(availableBalance)} Б
                    </span>
                </div>
            </div>
                ...
                <>
                    ...
                    <div className="grid grid-cols-[minmax(0,1fr)_auto_auto_auto] gap-2">
                        <div className="relative">
                            <input
                                value={spendAmount || ""}
                                onChange={(event) => setSpendAmount(parseAmount(event.target.value))}
                                disabled={isUnavailable}
                                inputMode="numeric"
                                placeholder="0"
                                className="h-12 w-full rounded-xl border border-gray-200 bg-white px-4 pr-9 text-base text-gray-900 outline-none transition placeholder:text-gray-400 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/10 disabled:bg-gray-100 disabled:text-gray-400 dark:border-white/10 dark:bg-white/5 dark:text-white dark:disabled:bg-white/5 dark:disabled:text-white/30"
                            />
                    ...
                    <div className="mt-3 rounded-xl border border-white/60 bg-white/70 p-3 dark:border-white/10 dark:bg-white/[0.04]">
                        ...
                        <div className="mt-2 flex items-center justify-between gap-3 text-sm text-emerald-700 dark:text-emerald-300">
                            ...
                            <span>-{formatAmount(spendAmount)} Б</span>
                        </div>
                    ...
                    <p className="mt-2 text-xs text-gray-500 dark:text-white/45">
                        {maxSpend <= 0
                            ? "Для этой записи бонусы списать нельзя: нет доступного баланса или стоимости."
